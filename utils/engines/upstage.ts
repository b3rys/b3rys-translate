// Solar-mini translation engine for Upstage AI.
// Uses OpenAI-compatible API format at https://api.upstage.ai/v1/chat/completions.
// Auth: Bearer token (Upstage API key).
...
export const upstageEngine: TranslationEngine = {
  async translate(apiKey, paragraphs, mode, subtitleContext, lang) {
    ...
    const content = data.choices?.[0]?.message?.content;
    if (!content) {
      throw new Error('Empty response from Upstage API');
    }
